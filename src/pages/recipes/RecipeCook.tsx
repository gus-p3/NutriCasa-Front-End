...
// Feedback interface removed as it is built inline
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Inicializar ingredientes desde la receta cruzando con la alacena
        if (recipeData.ingredients && recipeData.ingredients.length > 0) {
          
          const normalizeString = (str: string) => {
            if (!str) return '';
            let s = str.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim();
            // Muy básico para español: quitar 'es' o 's' al final para igualar plural/singular (ej. limones -> limon, huevos -> huevo)
          ...
          setIngredients(recipeData.ingredients.map((ing: any) => {
            const recipeIngName = normalizeString(ing.name);
            const inPantry = inventoryItems.find((i: any) => normalizeString(i.name) === recipeIngName);
            
            return {
              name: ing.name || '',
              quantity: ing.quantity || '',
              unit: ing.unit || 'piezas',
              isFromPantry: !!inPantry,
              pantryQuantity: inPantry ? inPantry.quantity : undefined,
              markedAsUsed: true, // Por defecto marcado como usado
              saveLeftover: false,
              leftoverQuantity: '',
              leftoverUnit: ing.unit || 'piezas'
            };
          }));
        } else {
          setIngredients([]);
  ...
  const handleSubmitFeedback = async () => {
    ...
    try {
      ...
      const feedbackData = {
        ...
        rating: rating,
        mealTime: mealTime,
        ingredientsUsed: ingredients.filter(ing => ing.markedAsUsed).map(ing => ({
          name: ing.name,
          quantityUsed: Number(ing.quantity) || 0,
          unit: ing.isFromPantry ? ing.unit : ing.leftoverUnit,
          isFromPantry: ing.isFromPantry,
          saveLeftover: ing.saveLeftover,
          leftoverQuantity: Number(ing.leftoverQuantity) || 0
        })),
        nota: personalNote.trim() || '',
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-3xl relative z-10">
          ...
          <div className="bg-white/90 backdrop-blur-md rounded-3xl shadow-2xl p-6 md:p-8 border border-white/50 relative overflow-hidden">
            ...
            <div className="relative">
              ...
              {ingredients.filter(i => i.isFromPantry).length > 0 && (
                <div className="mb-8">
                  <label className="block text-sm font-semibold text-gray-700 mb-4">
                    🍅 En tu alacena
                  </label>
                  <p className="text-xs text-gray-500 mb-3">Marca los que usaste para descontarlos de tu inventario.</p>
                  <div className="space-y-3">
                    {ingredients.map((ing, index) => {
                      if (!ing.isFromPantry) return null;
                      return (
                        <div 
                          key={index} 
                          className="group relative flex items-center gap-3 p-4 rounded-xl bg-gradient-to-r from-gray-50 to-white border border-gray-200 transition-all duration-300 hover:border-green-200"
                        >
                          <input
                            type="checkbox"
                            checked={ing.markedAsUsed}
                            onChange={(e) => handleIngredientChange(index, 'markedAsUsed', e.target.checked)}
                          ...
                          <div className="flex-1 flex flex-col sm:flex-row justify-between sm:items-center">
                            ...
                            <span className="text-sm text-gray-600">
                              Pedía: {ing.quantity} {ing.unit} 
                              <span className="text-xs text-green-600 ml-2">(Tienes {ing.pantryQuantity} {ing.unit})</span>
                            </span>
                          </div>
                        </div>
                      );
              ...
              {ingredients.filter(i => !i.isFromPantry).length > 0 && (
                <div className="mb-8">
                  ...
                  <p className="text-xs text-gray-500 mb-3">Marca si te sobró algo de los ingredientes que compraste y deseas guardarlo.</p>
                  <div className="space-y-4">
                    {ingredients.map((ing, index) => {
                      if (ing.isFromPantry) return null;
                      return (
                        <div key={index} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm transition-all duration-300">
                          <div className="flex items-center gap-3 mb-2">
                            <input
                              ...
                              className="h-5 w-5 text-green-600 rounded-lg focus:ring-green-500 transition-transform hover:scale-110"
                            />
                            <span className="font-medium text-gray-800 flex-1">{ing.name}</span>
                            <span className="text-sm text-gray-500">Pedía: {ing.quantity} {ing.unit}</span>
                          </div>
                          
                          {ing.markedAsUsed && (
                            <div className="pl-8 pt-3 border-t border-gray-100 flex flex-col gap-3 animate-fade-in">
                               <label className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                                 <input 
                                   type="checkbox" 
                                   checked={ing.saveLeftover}
                                   onChange={(e) => handleIngredientChange(index, 'saveLeftover', e.target.checked)}
                                   className="rounded text-green-600 focus:ring-green-500 w-4 h-4"
                                 />
                                 ¿Te sobró y deseas guardarlo en la alacena?
                               </label>
                               
                               {ing.saveLeftover && (
                                 <div className="flex gap-2 w-full sm:w-2/3 mt-1 animate-fade-in">
                                    <input
                                      type="number"
                                      min="0"
                                      step="0.1"
                                      placeholder="Cantidad que sobró"
                                      value={ing.leftoverQuantity}
                                      onChange={(e) => handleIngredientChange(index, 'leftoverQuantity', e.target.value)}
                                      className="flex-1 px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 text-sm"
                                    />
                                    <select
                                      value={ing.leftoverUnit}
                                      onChange={(e) => handleIngredientChange(index, 'leftoverUnit', e.target.value)}
                                      className="w-24 px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 text-sm"
                                    >
